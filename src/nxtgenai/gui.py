...
def main() -> None:
    ...
    cfg_path_obj = _resolve_config_path(cfg_path)

    cfg = configparser.ConfigParser(inline_comment_prefixes=("#", ";"))
    if cfg_path_obj.exists():
        cfg.read(cfg_path_obj)
        log.info("Loaded config from %s", cfg_path_obj)
    else:
        log.warning("config.ini not found at %s — using defaults.", cfg_path_obj)
