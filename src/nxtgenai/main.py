...
try:
    log_path = Path("/home/pi/reports/nxtgenai.log")
    ...
    # Keep stdout logging available even when file logging cannot be created.
    pass
...
class NxtGenAIApp:
    ...
    def _candidate_config_paths() -> list[Path]:
        ...
        return [
            ...
            here.with_name("config.ini"),
        ]

    @staticmethod
    def _load_config() -> configparser.ConfigParser:
        """Load config.ini from common repo/runtime locations."""
        cfg = configparser.ConfigParser(inline_comment_prefixes=("#", ";"))
        for cfg_path in NxtGenAIApp._candidate_config_paths():
...
<title>NxtGenAI Report — {ts}</title>
<style>{dark_css}</style>
</head>
<body>
<h1>🛡️ NxtGenAI Report</h1>
<p class="meta">Mode: <strong>{mode}</strong> &nbsp;|&nbsp; Generated: {ts}</p>
{results_html or '<p>No results recorded.</p>'}
<h2>Raw JSON</h2>
<pre>{json_dump}</pre>
<p class="meta">Generated by NxtGenAI — MIT License — For authorised use only.</p>
</body>
</html>"""
